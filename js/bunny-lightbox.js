/* Bunny HTML HLS Lightbox Player - standalone initializer */
(function(){
  'use strict';
  ...
  function initBunnyLightboxPlayer() {
    ...
    function planOnOpen(next) {
      ...
      if (!same) {
        ...
        attachMediaFor(next); autoStartOnReady = !!autoplay; pendingPlay = !!autoplay; return;
      }
      autoStartOnReady = !!autoplay;
      if (autoplay) { setStatus('loading'); safePlay(video); }
      else {
        try { if (!video.paused && !video.ended) video.pause(); } catch(_) {}
        setActivated(false); setStatus('paused');
      }
    }
    ...
    function togglePlay() {
      if (video.paused || video.ended) { pendingPlay = true; setStatus('loading'); safePlay(video); }
      else { video.pause(); }
    }
    ...
    function closeLightbox() {
      ...
      try { if (!video.paused && !video.ended) video.pause(); } catch (_) {}
      setActivated(false); setStatus(hasPlayed ? 'paused' : 'idle');
    }
